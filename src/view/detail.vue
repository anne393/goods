   ...
   <div @click="closeCart">
       <!--缩略图-->
		<div class="container g-piclist">
			<div class="g-return">
				<div class="text"><a href="shop.html">首页</a> > <span>{{oneGoods.name}}</span></div>
			</div>
			<div class="row">
				...
				<div class="u-content col-sm-5 col-xs-5">
					...
					<div class="m-buyma">
						<input type="button" class="f-buyBtn" value="加入购物车"  @click="addCart(1)"/>
						<input type="button" class="f-buyBtn" value="立即购买"  @click="addCart(2)" />
					</div>
				</div>
			</div>
...
<script>
import {goodslist} from "../datas/list.js"
...
export default {
	...
	methods:{
		addCart(flag){
			...
			this.$store.dispatch("addCartAction",this.oneGoods);
			if(flag==1){
				alert("加入到购物车")
			}
		},
